...
export APP_DOCKER_DIR=`pwd`
export VOLUME_DIR="$APP_DOCKER_DIR"

CONTAINER_TOOLBOX_ID=`docker ps --format '{{.ID}}\t{{.Names}}' | grep shortener_web_1 | cut -f1`
...
function command_boot()
{
    ...
    while [[ ! $(docker ps | grep shortener_web_1) ]] ; do
        echo -n "."
        sleep 1
    done
    echo ""
    echo "composer install --prefer-source --no-interaction" |  docker exec -i  shortener_web_1 /bin/bash
    echo ""
}

function command_tests(){
    echo " ./bin/phpunit tests" |  docker exec -i  shortener_web_1 /bin/bash
}
